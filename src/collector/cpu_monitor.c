...
static int parse_cpu_line(const char *line, int *index,
    ...
    char *end;
    int fields = 0;
    if (sscanf(line, "%15s", name) != 1 || strncmp(name, "cpu", 3) != 0)
        return 0;
    if (strcmp(name, "cpu") == 0) {
        *index = 0;
    } else {
        char *end = NULL;
        long core = strtol(name + 3, &end, 10);
        if (*end != '\0' || core < 0 || core >= CPU_MONITOR_MAX_CORES)
            return 0;
        *index = (int)core + 1;
    }
    ...
    *idle = values[3];
    if (fields >= 5) { *total += values[4]; *idle += values[4]; }
    if (fields >= 6) *total += values[5];
    if (fields >= 7) *total += values[6];
    if (fields >= 8) *total += values[7];
    return 1;
}

static double usage_percent(cpu_jiffies_t old, cpu_jiffies_t current) {
    if (current.total < old.total || current.idle < old.idle) return 0.0;
...
int cpu_monitor_sample(cpu_monitor_t *monitor, sensor_message_t *messages,
    ...
    if (!monitor || !messages || !message_count || capacity == 0) return -1;
    *message_count = 0;
    ...
    if (!monitor->initialized) {
        ...
        for (index = 1; index <= CPU_MONITOR_MAX_CORES; ++index)
            if (seen[index]) monitor->core_count++;
        return 1;
    }
    for (index = 0; index <= CPU_MONITOR_MAX_CORES; ++index)
        if (seen[index]) required++;
